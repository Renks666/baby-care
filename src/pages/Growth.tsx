    ...
    <>
            ...
            {records.map((r) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">{formatDate(r.date)}{r.notes ? ` · ${r.notes}` : ''}</p>
                </div>
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="Новый замер">
        <div className="space-y-3 pb-2">
          ...
          <div>
            <label className="text-xs text-gray-500 dark:text-gray-400 font-medium block mb-1">Комментарий</label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Например: после визита к педиатру"
              rows={2}
